...
def find_order_blocks(df: pd.DataFrame, lookback: int = OB_LOOKBACK) -> dict:
    ...
    if curr_close > recent_high and curr_body > (avg_body * OB_IMBALANCE_RATIO):
        for i in range(2, min(15, len(df))):
            ...
            # Bullish OB: แท่งแดง (p_close < p_open) และ low ของแท่งนั้นยังไม่ถูกทะลุในแท่งถัดมา
            if p_close < p_open and not (df["low"].iloc[idx + 1 : -1] < p_low).any():
        ...
        for i in range(2, min(15, len(df))):
            ...
            # [FIX Bug 4] Bearish OB: แท่งเขียว (p_close > p_open) และ high ของแท่งนั้นยังไม่ถูกทะลุในแท่งถัดมา
            # แก้ slice จาก idx+1: เป็น idx+1:-1 เพื่อไม่รวมแท่งปัจจุบัน (ตรงกับ Bullish OB logic)
            if p_close > p_open and not (df["high"].iloc[idx + 1 : -1] > p_high).any():
...
def scan_market():
    ...
    for coin in COINS:
        ...
        elif rsi >= RSI_OVERBOUGHT or (ob_info.get("has_bearish_ob") and ob_info.get("bearish_ob_price") is not None):
