...
# ==========================================
# RSI Bounce & Recovery Quality Check
# ==========================================
def analyze_rsi_bounce(df: pd.DataFrame) -> dict:
    ...
    # นำเงื่อนไข RSI Recovery Threshold มาเพิ่มแต้มความแข็งแกร่ง
...
# ==========================================
# Signal Logic (Updated with new parameters)
# ==========================================
def check_bullish_divergence(df: pd.DataFrame) -> bool:
    """
    เช็ก Bullish Divergence ตามพารามิเตอร์เซ็ตใหม่
    ข้ามแท่งล่าสุดตาม LOOKBACK_SKIP_BARS เพื่อหาจุดกลับตัวในอดีตที่เสถียรกว่า
    """
    if len(df) < LOOKBACK_BARS + 2:
        return False

    # กำหนด Window เก่า โดยการหักช่วง Skip ออกไปก่อน
    ...
    # เงื่อนไขข้อใหม่: RSI ของจุดต่ำสุดเก่าต้องไม่เกินพารามิเตอร์ที่กำหนด (rsi_bull_div_max = 45)
    ...
    # Bullish Divergence: ราคาทำ New Low แต่ RSI ยก Low สูงขึ้นกว่า Low เก่าใน window
    return (curr_price < prev_low_price) and (curr_rsi > prev_low_rsi)


def check_bearish_divergence(df: pd.DataFrame) -> bool:
    """
    (ฟังก์ชันเผื่อใช้ในอนาคต) เช็ก Bearish Divergence อิงตามพารามิเตอร์ใหม่
    """
    if len(df) < LOOKBACK_BARS + 2:
        return False
    ...
    prev_high_price = prev_window["high"].iloc[max_high_idx]
    prev_high_rsi   = prev_window["RSI"].iloc[max_high_idx]

    # เงื่อนไขข้อใหม่: RSI ของจุดสูงสุดเก่าต้องไม่ต่ำกว่าพารามิเตอร์ที่กำหนด (rsi_bear_div_min = 55)
    if prev_high_rsi < RSI_BEAR_MIN_MIN:
        return False

    curr_price = df["high"].iloc[-1]
    curr_rsi   = df["RSI"].iloc[-1]

    # Bearish Divergence: ราคาทำ New High แต่ RSI กลับลดต่ำลง
    return (curr_price > prev_high_price) and (curr_rsi < prev_high_rsi)
...
def scan_market():
    ...
    for coin in COINS:
        ...
        if current_price > ema_200:
            ...
            # ตรวจสอบการย่อตัวลงมา และเช็กด้วยว่าหลุดแนวรับสำคัญหรือฟื้นตัวจาก Pullback Threshold หรือไม่
            if current_price > (ema_50 * 0.98) and (rsi <= RSI_OVERSOLD or rsi <= RSI_PULLBACK_THRESHOLD):
                # หาก RSI ต่ำกว่า Pullback threshold และมีสัญญาณดีดกลับร่วมด้วย
...
def build_messages(buy_list: list, sell_list: list, market_summary: str) -> list:
    ...
    if buy_list:
        ...
        for opt in buy_list:
            ...
            # ควบคุมความยาวข้อความไม่ให้เกินลิมิตของ Telegram (4096 bytes เผื่อความปลอดภัยที่ 3500)
    ...
    # 3. เตือนโซน Overbought / พิจารณาแบ่งทำกำไร
    if sell_list:
        sell_header = (
            "⚠️ <b>[Crypto Screener 4H - เตือนโซน Overbought]</b>\n"
            "<i>คำแนะนำ: ราคาวิ่งแรงเกินไป ควรพิจารณาแบ่งขายทำกำไรตามโซนเป้าหมาย</i>"
        )
        ...
        for opt in sell_list:
            ...
            # แนวโน้มต่อเนื่องในโซน Overbought
            trend_block = (
                f"\n📐 <b>แนวโน้มต่อเนื่อง:</b> {ti['trend_label']}"
                f"\n   EMA50 slope: {ti['ema50_slope_pct']:+.3f}% | EMA200 slope: {ti['ema200_slope_pct']:+.3f}%"
            )
    ...
    # 4. กรณีไม่มีสัญญาณใด ๆ เกิดขึ้น
...
if __name__ == "__main__":
    logger.info("เริ่มต้น Crypto Screener (4H | Advanced Parameters Update Version)...")

    # สั่งแสกนหาข้อมูลและสัญญาณตลาด
    buy_list, sell_list, market_summary = scan_market()

    logger.info(f"สแกนเสร็จเรียบร้อย → ตรวจพบสัญญาณซื้อ: {len(buy_list)} | ตรวจพบโซนขาย: {len(sell_list)}")

    # สร้างชุดข้อความรายงานผล
    final_messages = build_messages(buy_list, sell_list, market_summary)
    
    # ส่งสัญญาณออกไปยังไลน์แชทบอท Telegram
    send_telegram_messages(final_messages)

    logger.info("ส่งรายงานเข้า Telegram เสร็จสมบูรณ์")
