...
OB_LOOKBACK = 20              # จำนวนแท่งย้อนหลังที่ใช้หาจุดสูงสุด/ต่ำสุดเพื่อเช็ก BOS
OB_IMBALANCE_RATIO = 1.5      # แท่งพุ่งออกจาก OB ต้องยาวกว่าค่าเฉลี่ยกี่เท่า (เนื้อแท่งเทียนแน่นแน่น)

# --- Take Profit & Stop Loss Tiers (Optimized for 4H Timeframe) ---
TP_TIERS = {
    "major":  {"tp1": 0.10, "tp2": 0.15, "sl_buffer": 0.025},  # SL ~2.5% เน้นความแม่นยำสูง
    "mid":    {"tp1": 0.15, "tp2": 0.20, "sl_buffer": 0.050},  # SL ~5.0% เผื่อแรงสะบัดเคลียร์เลเวอเรจ
    "small":  {"tp1": 0.25, "tp2": 0.35, "sl_buffer": 0.080},  # SL ~8.0% รองรับความซิ่งของเหรียญเล็ก
}
...
# ==========================================
# Smart Money / Structure & Signal Logic
# ==========================================
def find_order_blocks(df: pd.DataFrame, lookback: int = OB_LOOKBACK) -> dict:
    ...
    if curr_close > recent_high and curr_body > (avg_body * OB_IMBALANCE_RATIO):
        for i in range(2, min(15, len(df))):
            ...
            if p_close < p_open: # แท่งสีแดงก่อนระเบิดราคา
                subsequent_lows = df["low"].iloc[idx+1:]
                if not (subsequent_lows < p_low).any(): # ยังไม่ถูกทำลาย (Mitigated)
    ...
    # 2. Bearish Order Block (BOS Breakdown)
        ...
        for i in range(2, min(15, len(df))):
            ...
            if p_close > p_open: # แท่งสีเขียวก่อนโดนทุบ
...
if __name__ == "__main__":
    logger.info("เริ่มต้นใช้งาน Crypto Screener 4H (SMC Order Block + Rebound v4)...")

    # 1. ทำการสแกนหาจุดซื้อขายและคำนวณเทรนด์รวม
    ...
    # 2. แปลงผลลัพธ์ที่สแกนได้ให้ออกมาเป็นบล็อกข้อความ HTML
    final_messages = build_messages(buy_list, sell_list, market_summary)
    
    # 3. ยิงข้อความแจ้งเตือนตรงเข้าแอป Telegram
    send_telegram_messages(final_messages)

    logger.info("บอททำงานและส่งข้อมูลรายงานตลาดเสร็จสมบูรณ์!")
