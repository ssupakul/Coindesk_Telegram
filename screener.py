...
# อัปเดตโครงสร้างการทำกำไร (Take Profit) เป็น 2 ระดับตามกลุ่มเหรียญ
TP_TIERS = {
...
def scan_market():
    ...
    for coin in COINS:
        ...
        if signal_type:
            ...
            target_profit1 = format_price(current_price * (1 + tp1_pct))
            target_profit2 = format_price(current_price * (1 + tp2_pct))
            ...
            buy_signals.append(
                {
                    ...
                    "tp":            f"🎯 TP1: ${target_profit1} (+{tp1_pct*100:.0f}%)\n🎯 TP2: ${target_profit2} (+{tp2_pct*100:.0f}%)",
                    "sl":            f"${stop_loss}",
